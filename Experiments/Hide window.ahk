;This HideWindow app quickly makes a window disappear, WIN+H (+H), and even removes the active Taskbar icon making it almost impossible to find the hidden window. It's not minimized, but hidden. Great for hiding inappropriate activities from the boss. The hidden window continues to run the program (whatever it is) in the background. Right-click on the AutoHotkey icon in the System Tray and select if from the menu to bring it back.
